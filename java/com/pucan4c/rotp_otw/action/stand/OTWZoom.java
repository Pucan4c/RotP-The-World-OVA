...
public class OTWZoom extends StandEntityAction {

    public OTWZoom(StandEntityAction.Builder builder) {
    ...
    public void onHoldTickClientEffect(LivingEntity user, IStandPower power, int ticksHeld, boolean reqFulfilled, boolean reqStateChanged) {
        if (reqStateChanged && reqFulfilled) {
            ClientTickingSoundsHelper.playHeldActionSound(InitSounds.OTW_ZOOM.get(),
            ...
            if (ticks % 16 == 3 && ticks > 32 && ticks < 80) {
                ...
                if (player.is(standEntity.getUser())) {
                    world.playSound(player, standEntity.getX(), standEntity.getY(), standEntity.getZ(), 
                            InitSounds.OTW_ZOOM_CLICK.get(), standEntity.getSoundSource(), 1.0F, 1.0F);
                }
            }
        }
    }
