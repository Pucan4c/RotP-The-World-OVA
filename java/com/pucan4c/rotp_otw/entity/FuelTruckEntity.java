package com.pucan4c.rotp_otw.entity;
...
import com.pucan4c.rotp_otw.init.InitEntityTypes;
import com.pucan4c.rotp_otw.init.InitSounds;
import com.pucan4c.rotp_otw.util.mc.DamageUtil;
...
public class FuelTruckEntity extends Entity implements IHasHealth {
    private static final DataParameter<Float> HEALTH = EntityDataManager.defineId(FuelTruckEntity.class, DataSerializers.FLOAT);
    ...
    public FuelTruckEntity(World world) {
        this(InitEntityTypes.FUEL_TRUCK.get(), world);
    }

    public FuelTruckEntity(EntityType<FuelTruckEntity> type, World world) {
        super(type, world);
    }
    ...
    public void tick() {
        ...
        DamageSource dmgSource = DamageUtil.fuelTruckDamage(this);
        ...
        if (level.isClientSide() && !wasOnGround) {
            ticksInAir++;
            if (onGround) {
                level.playSound(ClientUtil.getClientPlayer(), getX(), getY(), getZ(), InitSounds.FUEL_TRUCK_LAND.get(),
    ...
    public boolean hurt(DamageSource dmgSource, float amount) {
        if (isInvulnerableTo(dmgSource)) {
            ...
            if (!level.isClientSide()) {
                ...
                markHurt();
                level.playSound(null, getX(), getY(), getZ(), InitSounds.FUEL_TRUCK_HIT.get(),
